...
recursive arguments (`Acc`), and an empty type (`False`).
-/
...
run_meta do
  ...
  if ∃ j < (2 : Nat), rhs.RuleShape 0 1 2 0 j then
    throwError "RuleShape accepts nf = 0 for Nat.rec/Nat.succ"
  if ∃ j < (1 : Nat), rhs.RuleShape 0 1 1 1 j then
  ...
  -- minor premise for `Nat.succ`; a reduct whose recursive-call slot holds garbage is not
  -- rejected by the shapes (it is rejected by `rules_wf`, a typing).
  ...
  -- (The same template is shape-correct as a rule for `Nat.zero` with one field: the field
  -- count is checked against the constructor's type by `rules_ctor`, not by the shapes.)
  unless ruleShapeAt ty bogusSuccRhs 0 1 2 1 ``Nat.zero do
    throwError "rule_shape rejects the zero minor for Nat.rec/Nat.zero"
